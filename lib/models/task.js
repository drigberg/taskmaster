...
class TaskData {
    ...
    static partialToDynamoDB(userId, taskId, data) {
        ...
        if (providedKeys.includes('archived')) {
            payload.acrhived = { BOOL: data.archived };
        }
        return payload;
    }
...
class Task {
    ...
    static async updateCompletionDates(userId, taskId, data) {
        // TODO: validate userId in authorization
        ...
        await dynamodb.updateItem({
            ...
            ExpressionAttributeValues: {
                ':v1': payload.completionDates
            }, 
            UpdateExpression: 'SET completionDates=:v1'
        }).promise();
        // TODO: handle errors
    ...
    static async addCompletionDate(userId, taskId, completionDate) {
        ...
        if (!taskData.completionDates.includes(completionDate)) {
            ...
            taskData = await this.updateCompletionDates(userId, taskId, { completionDates: taskData.completionDates });
        }
