...
function handleCreateTask(req, res, next) {
    // TODO: protect route with authentication and authorization
    logger.info('Request received: handleCreateTask');
    const userId = req.params.id;
    try {
 ...
 * @param {Function} next - callback
 */
function handleUpdateUserTasks(req, res, next) {
    // TODO: protect route with authentication and authorization
    logger.info('Request received: handleUpdateUserTasks');
    const userId = req.params.id;
    try {
        const userData = store.getUserById(userId);
        res.json(userData);
...
function createApp() {
    ...
    app.post('/api/users/:id/updateTasks', handleUpdateUserTasks);
